// Package sonarr starts the Sonarr pollers on top of the shared arr client.
package sonarr
...
func Start(ctx context.Context, cfg config.AppConfig, s *snapshot.Store, m *metrics.M, notify func(string, bool)) *arr.Client {
	...
	arrapp.StartCommon(ctx, app, c, &s.Sonarr, m, notify)
	go poll.Run(ctx, arrapp.Spec(app, "library", 15*time.Minute, notify), &s.SonarrSeries, m, c.Series)
	return c
}
